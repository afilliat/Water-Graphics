...
void CSCI441::OpenGLUtils::printOpenGLInfo() {
	...
	GLint maxTransformFeedbackBuffers = 0;
	glGetIntegerv(GL_MAX_TRANSFORM_FEEDBACK_BUFFERS, &maxTransformFeedbackBuffers);

	GLint maxTransformSeparateComponents= 0;
	glGetIntegerv(GL_MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS, &maxTransformSeparateComponents);

	GLint maxTransformInterleavedComponents = 0;
	glGetIntegerv(GL_MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS, &maxTransformInterleavedComponents);
	...
	fprintf( stdout, "[INFO]: |   Max # Transform Feedback Buffers:  %17d |\n", maxTransformFeedbackBuffers );
	fprintf( stdout, "[INFO]: |   Max # Transform Separate Components:  %14d |\n", maxTransformSeparateComponents );
	fprintf( stdout, "[INFO]: |   Max # Transform Interleaveed Buffers:  %13d |\n", maxTransformInterleavedComponents );
	fprintf( stdout, "[INFO]: >--------------------------------------------------------<\n");
	...
	fprintf( stdout, "[INFO]: |   Max # Color Attachments:  %26d |\n", maxAttach );
	fprintf( stdout, "[INFO]: |   Max # Draw Buffers:  %31d |\n", maxDrawBuf );
	fprintf( stdout, "[INFO]: \\--------------------------------------------------------/\n\n");
}
